...
export default function Header({
  ...
  return (
    <header className="relative z-30 w-full p-3 sm:p-6 flex items-start justify-between gap-3 select-none">
      {/* Left Box: Live Listeners & Time */}
      <div className="bg-[#120e0b]/85 backdrop-blur-md border border-white/10 p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-xl flex flex-col gap-1.5 flex-shrink-0 min-w-[150px] sm:min-w-[170px] max-w-[210px] sm:max-w-[250px] z-20">
        {/* Live status badge */}
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] sm:text-xs font-mono font-bold tracking-wider text-amber-400 uppercase truncate">
            {listenerCount} Listening to Rain
          </span>
        </div>

        {/* Live Clock */}
        <div className="flex items-baseline gap-2">
          <span className="text-xl sm:text-2xl font-bold font-mono text-white tracking-tight tabular-nums">
            {formattedTime}
          </span>
        ...
        <div className="flex items-center gap-1.5 text-[10px] text-white/60 font-mono border-t border-white/5 pt-1.5 mt-0.5 min-w-0">
          ...
          <span className="truncate">{cityName} • {periodName}</span>
        </div>
      </div>

      {/* Center Title - Absolutely locked to horizontal center */}
      <div className="hidden md:flex absolute left-1/2 top-3 sm:top-5 -translate-x-1/2 flex-col items-center justify-center text-center pointer-events-none z-10">
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 flex-shrink-0 z-20 flex-wrap justify-end">
        ...
        <div className="relative">
          <button
            ...
            className={`px-3 py-2 rounded-xl text-xs font-mono border transition-all flex items-center gap-1.5 cursor-pointer sm:min-w-[110px] justify-center ${
          ...
          {showCityMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-[#18110c] border border-white/15 rounded-xl shadow-2xl p-2 z-50 animate-in fade-in zoom-in-95">
        ...
        {hasMultipleAngles && onToggleVariant && (
          <button
            ...
            title={`Switch scene view (${variantIndex + 1}/${timePeriod?.bgImages?.length})`}
          >
            <Camera className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" />
            <span className="hidden sm:inline truncate">
              Angle {variantIndex + 1}/{timePeriod?.bgImages?.length}
            </span>
          </button>
        )}
        ...
        <div className="relative">
          <button
            ...
            className={`px-3 py-2 rounded-xl text-xs font-mono border transition-all flex items-center gap-1.5 cursor-pointer sm:min-w-[120px] max-w-[160px] sm:max-w-[190px] justify-center ${
              currentOverride
          ...
          >
            ...
            <span className="hidden sm:inline truncate">
              {currentOverride ? scenesList[currentOverride]?.name : 'Live Time'}
            </span>
          </button>

          {/* Time Menu Dropdown */}
          {showTimeMenu && (
            <div className="absolute right-0 mt-2 w-56 bg-[#18110c] border border-white/15 rounded-xl shadow-2xl p-2 z-50 animate-in fade-in zoom-in-95">
              <div className="px-2.5 py-1 text-[9px] font-mono uppercase tracking-widest text-amber-400 border-b border-white/10 mb-1">
        ...
        {onOpenDiary && (
          <button
            onClick={onOpenDiary}
            className="bg-[#120e0b]/85 hover:bg-[#1a1410] border border-white/10 text-white/80 hover:text-amber-400 p-2 sm:px-3 sm:py-2 rounded-xl text-xs font-mono flex items-center justify-center gap-1.5 transition-colors cursor-pointer w-9 sm:w-[85px] flex-shrink-0"
        ...
        <button
          onClick={handleShare}
          className="bg-[#120e0b]/85 hover:bg-[#1a1410] border border-white/10 text-white/80 hover:text-amber-400 p-2 sm:px-3 sm:py-2 rounded-xl text-xs font-mono flex items-center justify-center gap-1.5 transition-colors cursor-pointer w-9 sm:w-[115px] flex-shrink-0"
        ...
        >
          ...
          <span className="hidden sm:inline truncate">{copied ? 'Link Copied' : 'Share'}</span>
        </button>
